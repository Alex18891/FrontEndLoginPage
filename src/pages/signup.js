...
export default function Signup()
{
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      if(!name || !username || !email || !pw || !confipw)
      {
        document.getElementById('textmatch1').innerHTML = 'Fields are required';
      }
      else{
      ...
      <div className="Appsignup">
        <header className="App-headersignup">
          ...
          <div className='App_main' > 
              ...
              <input type="password"  placeholder="Enter Password" onKeyUp={check} id="password" onChange={(e)=>setpw(e.target.value)} value={pw} required/>   
              <p className='text2'>Confirm Password</p>
              <input type="password"  placeholder="Enter Password to confirm" onKeyUp={check} id="confirm_password"  onChange={(e)=>setconfipw(e.target.value)} value={confipw} required/>        
